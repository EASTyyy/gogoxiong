 ...
 */
function doInitCitySelectXlclWeixin(provinceSelId,citySelId,districtSelId,currentProvinceId,currentcityId,currentdistrictId){
	...
	jQuery.get("/tjsc/weixin/address/getProvince",{},function(province_arr){
		...
		$("#"+provinceSelId).change(function(){
			...
			cityDom.options[cityDom.length] = new Option("-选择地区-","");
			if($(this).val()!=""){
				jQuery.get("/tjsc/weixin/address/getCity",{provinceId:$(this).val()},function(data){
		...
		$("#"+citySelId).change(function(){
			...
			if($(this).val()!=""){
				districtDom.options[districtDom.length] = new Option("-选择县市区-","");
				jQuery.get("/tjsc/weixin/address/getDistrict",{cityId:$(this).val()},function(data){
					for(var i=0;i<data.length;i++){
		...
		if(typeof currentProvinceId!='undefined'){
			...
			jQuery.get("/tjsc/weixin/address/getCity",{provinceId:currentProvinceId},function(data){
				for(var i=0;i<data.length;i++){
					cityDom.options[cityDom.length] = new Option(data[i].name,data[i].id);
				}
				if(typeof currentcityId!='undefined'){
					cityDom.value=currentcityId;
					jQuery.get("/tjsc/weixin/address/getDistrict",{cityId:currentcityId},function(data){
						for(var i=0;i<data.length;i++){
							districtDom.options[districtDom.length] = new Option(data[i].name,data[i].id);
						}
