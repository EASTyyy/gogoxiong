 ...
 */
function doInitCitySelect(provinceSelId,citySelId,districtSelId,currentProvinceId,currentcityId,currentdistrictId){
	...
	jQuery.get("/common/city/getProvinces",{},function(province_arr){
		...
		$("#"+provinceSelId).change(function(){
			...
			cityDom.options[cityDom.length] = new Option("-选择地区-","");
			if($(this).val()!=""){
				jQuery.get("/common/city/getCitys",{provinceId:$(this).val()},function(data){
		...
		$("#"+citySelId).change(function(){
			...
			if($(this).val()!=""){
				districtDom.options[districtDom.length] = new Option("-选择县市区-","");
				jQuery.get("/common/city/getDistricts",{cityId:$(this).val()},function(data){
					for(var i=0;i<data.length;i++){
		...
		if(typeof currentProvinceId!='undefined'){
			...
			jQuery.get("/common/city/getCitys",{provinceId:currentProvinceId},function(data){
				for(var i=0;i<data.length;i++){
					cityDom.options[cityDom.length] = new Option(data[i].value,data[i].id);
				}
				if(typeof currentcityId!='undefined'){
					cityDom.value=currentcityId;
					jQuery.get("/common/city/getDistricts",{cityId:currentcityId},function(data){
						for(var i=0;i<data.length;i++){
							districtDom.options[districtDom.length] = new Option(data[i].value,data[i].id);
						}
