...
jQuery.extend(jQuery.fn.validatebox.defaults.rules,{
	phone : {
		...
		message : typeof param != 'undefined' && typeof param[0] != 'undefined' ? param[0]:"请输入有效的联系电话"
	}
});
...
jQuery.extend(jQuery.fn.validatebox.defaults.rules, {
	mobile : {
		validator : function(value, param) {
			var exp = /^((1[1,3,5,7,8]{2})+\d{8})$/;
			return isMatchedRegx(value, exp);
		},
		message : typeof param != 'undefined' && typeof param[0] != 'undefined' ? param[0]:"请输入有效的手机号码"
	}
});
...
function outPutFunction(_functionName,_objId,_linkname,_permcode){
	return '[<a style="color:blue;" href="javascript:'+_functionName+'('+_objId+')">'+_linkname+'</a>]';
}
...
function doResetSearchObject(_tbid, _formid){
	$("#"+_formid+" input[type='reset']").trigger("click");
	var array = $("#"+_formid+" .easyui-combotree");
	console.log(array);
	for (var i = array.length - 1; i >= 0; i--) {
		$(array[i]).combotree('reset');
	};
	var array1 = $("#"+_formid+" .easyui-combogrid");
	for (var i = array1.length - 1; i >= 0; i--) {
		$(array1[i]).combogrid('reset');
	};
	var queryParams = $('#'+_tbid).datagrid('options').queryParams; 
 	$('#'+_tbid).datagrid('options').queryParams=mergeFormToJson(queryParams,_formid);
	$("#"+_tbid).datagrid('reload');
...
function doSearchObjectForTree(_tbid,_formid){
	 var queryParams = $('#'+_tbid).treegrid('options').queryParams; 
	 $('#'+_tbid).treegrid('options').queryParams=mergeFormToJson(queryParams,_formid);
